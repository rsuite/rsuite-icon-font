const svgr = require('@svgr/core').default;
const fs = require('fs-extra');
const glob = require('glob');
const path = require('path');
const camelCase = require('camelcase');

const SOURCE_DIR = './src/svg';
const DIST_DIR = './dist/components';

fs.mkdirpSync(DIST_DIR);

const files = glob.sync(`${SOURCE_DIR}/**/*.svg`);

files.forEach(function generateComponent(svgPath, index) {
  const basename = path.basename(svgPath);
  const componentName = camelCase(
    path.basename(basename.replace(/u\w+-/, ''), path.extname(basename)),
    {
      pascalCase: true,
    }
  );
  const svg = fs.readFileSync(svgPath, 'utf8');
  console.log(`(${index + 1}/${files.length}) Generating ${componentName}.tsx ...`);
  const jsCode = svgr.sync(
    svg,
    {
      plugins: ['@svgr/plugin-svgo', '@svgr/plugin-jsx', '@svgr/plugin-prettier'],
      typescript: true,
      prettier: true,
      svgo: true,
      expandProps: 'start',
      svgProps: {
        width: '1em',
        height: '1em',
        fill: 'currentColor',
      },
    },
    { componentName }
  );
  fs.outputFileSync(path.join(DIST_DIR, `${componentName}.tsx`), `// Generated by script, don't edit it please.
${jsCode}`);
});
